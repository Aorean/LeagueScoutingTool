...
from backend.functions.api import get_gamertag_tagline
...
def json_player(player, db_connection):
    return_dict = {}
    return_dict["player"] = []
    for puuid in player:
        
        query_player = get_query("select_json", schema="playerdata", table="player", selection="puuid", value=puuid)
        query_playerinfo = get_query("select_json", schema="playerdata", table="playerinfo", selection="puuid", value=puuid)
        
        
        data_player = execute_query(db_connection, query_player)
        data_playerinfo = execute_query(db_connection, query_playerinfo)

        filtered_player = data_player[0][0][0]
        filtered_playerinfo = data_playerinfo[0][0][0]

        combined_player = {}
        for key in filtered_player:
            data = filtered_player[key]
            combined_player[key] = data

        for key in filtered_playerinfo:
            data = filtered_playerinfo[key]
            combined_player[key] = data

        combined_player["matchhistory"] = []



        return_dict["player"].append(combined_player)
...
def json_tcmatches(player, db_connection):
    print("placeholder")
