...
from def_func import get_roleopponent, get_playerscouting, get_df_playerinfo, get_df_player
...
for account in riot_gamenames_gametags:
    ...
    #player dataframe get
    temp_Dataframe_Player = get_df_player(puuid_player, account)
    #player dataframe concat
...
for player in puuids:
    ...
    #df player_info get
    temp_Dataframe_Matchhistory = get_df_playerinfo(player, matchhistories, rank_datas)
    #df player_info concat
    Dataframe_Player_Info = pd.concat([Dataframe_Matchhistory, temp_Dataframe_Matchhistory])
    ...
    for match in match_history_for_matchdata:
        ...
        for player in participant_dto:
            if puuid == player["puuid"]:
                player_scouting = get_playerscouting(player, info)
        ...
        for player in participant_dto:
            if (player_scouting["position"] == player["teamPosition"] and player_scouting["team"] != player[
                "teamId"]):
                role_opponent = get_roleopponent(player, info)

                puuid_opponent.append(role_opponent["puuid"])

                #getting deltas of player vs opponent
